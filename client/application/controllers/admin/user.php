...
class user extends Admin_Controller
{
    ...
    {
		if(!empty($_GET['code'])) {
			$url = 'http://'.$_SERVER['HTTP_HOST'].'/admin/user/login';
			$user_id = get_vk_id($_GET['code'],$url);
			if(empty($user_id)) die(show_error('Ошибка авторизации.'));
			$user = $this->users_model->get_by(array('vk_id' => $user_id),true);
			if(empty($user->id)) die('error');
			 $data = array('name' => $user->name,'email' => $user->email,'id' => $user->id,'group' => $user->group,'loggedin' => TRUE);
  ...
  KEY `word` (`word`)
        ...
        $vals   = array(
            ...
            'font_path' => getcwd() . '/system/fonts/Roboto-Black.ttf', // шрифт, которым будет написана капча
            'img_width' => 260, // ширина капчи
            'img_height' => 45, // высота капчи
    ...
    public function isBlocked()
    {
        ...
        //
            ...
            //
                ...
                //
                    $this->db->where('ip_address', $ip_address)->update('login_attempts', array(
                        'attempts' => 0,
                        'lastLogin' => time()
                    ));
                    return false;
