    ...
    switch ($xfieldssubaction) {
      ...
      case "edit":
        ...
        if (!$editedxfield) {
		  ...
		  $editedxfield[1] = htmlspecialchars( strip_tags( stripslashes( trim( $editedxfield[1] ) ) ), ENT_QUOTES, $config['charset'] );
		  $editedxfield[18] = htmlspecialchars( strip_tags( stripslashes( trim( $editedxfield[18] ) ) ), ENT_QUOTES, $config['charset'] );
...
<div class="box">
  ...
  <div class="box-content">
	...
	<div class="row box-section">
		...
		<div class="form-group">
		  <label class="control-label col-md-2"><?php echo $lang['xfield_hint']; ?></label>
		  <div class="col-md-10">
			<input style="width:100%;max-width: 350px;" type="text" name="editedxfield[18]" value="<?php echo $editedxfield[18];?>" placeholder="<?php echo $lang['xfield_hint_1']; ?>" />
		  </div>
		 </div>
...
echo <<<HTML
		...
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['xf_group_add']}</label>
		  <div class="col-md-10">
		...
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['xf_group_view']}</label>
		  <div class="col-md-10">
...
?>
		...
		<div id="default_image">		
			...
			<div id="optional5" class="form-group">
			  <label class="control-label col-md-2"><?php echo $lang['xfield_xi9']; ?></label>
			  <div class="col-md-10">
				<input style="width:100%;max-width: 100px;" type="text" name="editedxfield[16]" value="<?php echo htmlspecialchars($editedxfield[16], ENT_QUOTES, $config['charset']); ?>" />&nbsp;<span class="help-button" data-rel="popover" data-trigger="hover" data-placement="right" data-content="<?php echo $lang['xfield_xi10']; ?>" >?</span>
			  </div>
			</div>
    ...
    foreach ($xfields as $name => $value) {
	  ...
	  if ($xfieldmode == "site") {
		...
		if ($value[18]) {
			...
			$help_text = $value[18];
			$value[18] = "&nbsp;<span class=\"help-button\" data-rel=\"popover\" data-trigger=\"hover\" data-placement=\"right\" data-content=\"{$value[18]}\" >?</span>";
      ...
      if ($value[3] == "textarea") {      
		...
		if ($xfieldmode == "site") {
	        ...
	        $output .= <<<HTML
<tr id="$holderid" {$uid}>
<td class="xfields" colspan="2">{$value[1]}: [not-optional]<span style="color:red;">*</span>[/not-optional]<div class="{$class_name}">{$panel}<textarea name="xfield[$fieldname]" id="xf_$fieldname" {$params}>$fieldvalue</textarea>{$value[18]}</div></td></tr>
HTML;
...
<div id="$holderid" class="form-group editor-group" {$uid}>
  <label class="control-label col-lg-2">{$value[1]}: [not-optional]<span style="color:red;">*</span>[/not-optional]{$value[18]}</label>
  <div class="col-lg-10">
...
<div id="$holderid" class="form-group" {$uid}>
  <label class="control-label col-md-2">{$value[1]}: [not-optional]<span style="color:red;">*</span>[/not-optional]</label>
  <div class="col-md-10">
     <input type="text" style="width:99%;max-width:437px;" name="xfield[$fieldname]" id="xf_$fieldname" value="$fieldvalue" {$params}/> {$value[18]}
  </div>
</div>
HTML;
...
<div id="$holderid" class="form-group">
  ...
  <div class="col-md-10">{$select} {$value[18]}
  </div>
</div>
HTML;
		}
...
HTML;
		...
		$xfieldinput[$fieldname] = $select;
			...
			if ($value[18]) $value[18] = "&nbsp;<span class=\"help-button\" style=\"position: relative;top: -8px;\" data-rel=\"popover\" data-trigger=\"hover\" data-placement=\"right\" data-content=\"{$help_text}\" >?</span>";
	
			$output .= <<<HTML
<div id="$holderid" class="form-group">
  <label class="control-label col-md-2">{$value[1]}:</label>
  <div class="col-md-10"><input class="iButton-icons-tab" type="checkbox" name="xfield[$fieldname]" value="1" {$selected}>{$value[18]}
  </div>
</div>
HTML;
		}		
...
<div id="$holderid" class="form-group" {$uid}>
  ...
  <div class="col-md-10"><div id="xfupload_{$fieldname}"></div><input type="hidden" name="xfield[$fieldname]" id="xf_$fieldname" value="{$fieldvalue}" {$params}/>
<script type="text/javascript">
jQuery(function($){
{$uploadscript}
});
...
<div id="$holderid" class="form-group" {$uid}>
  <label class="control-label col-md-2">{$value[1]}: [not-optional]<span style="color:red;">*</span>[/not-optional] {$value[18]}</label>
  <div class="col-md-10"><input style="width:99%;max-width:250px;margin-bottom:5px;{$show}" type="text" name="xfield[$fieldname]" id="xf_$fieldname" value="{$fieldvalue}" {$params}/><span id="uploadedfile_{$fieldname}">{$fileid}</span><div id="xfupload_{$fieldname}"></div>
<script type="text/javascript">
jQuery(function($){
{$uploadscript}
});
  ...
  case "templatereplacepreview":
    ...
    foreach ($xfields as $value) {
	  ...
	  $parse->allow_code = true;
		...
		if (($value[8] == 1 OR $value[3] == "select" OR $value[3] == "image" OR $value[3] == "imagegalery" OR $value[3] == "file" ) AND $xfield[$value[0]] != "" ) {
