		...
		function check_files ( folder ){
			...
			if (folder == "snap") {
				...
				DLEconfirm( '{$lang['anti_snapalert']}', '{$lang['p_confirm']}', function () {
					...
					ShowLoading('');		
					$.post('engine/ajax/antivirus.php', { folder: folder, user_hash: '{$dle_login_hash}' }, function(data){
		...
		$(function(){
			...
			$('#clearsubscribe').click(function() {
			    ...
			    DLEconfirm( '{$lang['confirm_action']}', '{$lang['p_confirm']}', function () {
					...
					$.get("engine/ajax/adminfunction.php?action=clearsubscribe&user_hash={$dle_login_hash}", function( data ){
						Growl.info({
							title: '{$lang[p_info]}',
							text: data
						});
			...
			$('#check_updates').click(function() {
				...
				$.get("engine/ajax/updates.php?versionid={$config['version_id']}&user_hash={$dle_login_hash}", function( data ){
					$('#main_box').html(data);
				});
				return false;
			});
			...
			$('#send_notice').click(function() {
				...
				$.post("engine/ajax/adminfunction.php?action=sendnotice&user_hash={$dle_login_hash}", { notice: notice } , function( data ){
		...
		<div class="box">
		    ...
		    <div class="box-header">
				<ul class="nav nav-tabs nav-tabs-left">
					...
					<li id="dlestats"><a href="#statauto" data-toggle="tab"><i class="icon-cog"></i> {$lang['stat_auto']}</a></li>
				</ul>
			</div>
...
HTML;
	...
	if( !@extension_loaded('xml') ) {
		echo "<div class=\"alert alert-error\">{$lang['stat_not_min']} XML</div>";
	}

	if( !@extension_loaded('zlib') ) {
		echo "<div class=\"alert alert-error\">{$lang['stat_not_min']} Zlib</div>";
	}
