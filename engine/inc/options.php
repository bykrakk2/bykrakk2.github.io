...
function del_mod ( id ){
    ...
    DLEconfirm( '{$lang['confirm_del_mod']}', '{$lang['p_confirm']}', function () {
		...
		$.get("engine/ajax/adminfunction.php?action=deletemodules&user_hash={$dle_login_hash}", { id: id }, function( data ){
...
elseif( $action == "personal" ) {
	...
	$registrationdate = langdate( $langformatdatefull, $member_id['reg_date'] ); //registration date
...
elseif( $action == "dosavepersonal" ) {
	...
	if( strlen($editpassword) > 72 ) $editpassword = substr($editpassword, 0, 72);
	...
	if( $editpassword != "" ) {
		...
		if( password_verify($altpass, $member_id['password'] ) ) {
			...
			$editpassword = $db->safesql( password_hash($editpassword, PASSWORD_DEFAULT) );
		...
		} else msg( "error", $lang['user_err'], $lang['opt_errpass'], "?mod=options&action=personal" );
...
HTML;
	...
	showRow( $lang['opt_sys_path'], $lang['opt_sys_pathd'], "<input type=\"text\" name=\"save_con[admin_path]\" value=\"{$config['admin_path']}\" style=\"width:100%;\">", "white-line" );
	...
	showRow( $lang['opt_sys_recaptpub'], $lang['opt_sys_recaptpubd'], "<input  type=\"text\" name=\"save_con[recaptcha_public_key]\" value=\"{$config['recaptcha_public_key']}\" style=\"width:100%;\">" );
	showRow( $lang['opt_sys_recaptpriv'], $lang['opt_sys_recaptpubd'], "<input  type=\"text\" name=\"save_con[recaptcha_private_key]\" value=\"{$config['recaptcha_private_key']}\" style=\"width:100%;\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_findr'], $lang['opt_sys_findrd'], "<input type=\"text\" style=\"text-align: center;\"  name=\"save_con[search_pages]\" value=\"{$config['search_pages']}\" size=\"20\">" );
	...
	showRow( $lang['opt_sys_an'], "<a onclick=\"javascript:Help('date'); return false;\" class=main href=\"#\">$lang[opt_sys_and]</a>", "<input  type=\"text\" name=\"save_con[timestamp_active]\" value=\"{$config['timestamp_active']}\" size=\"20\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_ct'], "<a onClick=\"javascript:Help('date')\" class=main href=\"#\">$lang[opt_sys_and]</a>", "<input  type=\"text\" name='save_con[timestamp_comment]' value=\"{$config['timestamp_comment']}\" size=\"20\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_mcac'], $lang['opt_sys_mcacd'], "<input  type=\"text\" style=\"text-align: center;\" name=\"save_con[max_cache_pages]\" value=\"{$config['max_cache_pages']}\" size=\"20\">" );
	showRow( $lang['opt_sys_fc'], $lang['opt_sys_fcd'], "<input  type=\"text\" style=\"text-align: center;\" name=\"save_con[fullcache_days]\" value=\"{$config['fullcache_days']}\" size=\"20\">" );
	...
	showRow( $lang['opt_sys_memserv'], $lang['opt_sys_memservd'], "<input  type=\"text\" name=\"save_con[memcache_server]\" value=\"{$config['memcache_server']}\" style=\"width:100%;\"" );
...
HTML;
	...
	showRow( $lang['opt_sys_amail'], $lang['opt_sys_amaild'], "<input  type=\"text\" name='save_con[admin_mail]' value='{$config['admin_mail']}' style=\"width:100%;max-width:250px\">", "white-line" );
	showRow( $lang['opt_sys_mt'], $lang['opt_sys_mtd'], "<input  type=\"text\" name='save_con[mail_title]' value=\"{$config['mail_title']}\" style=\"width:100%;max-width:250px\">" );

	showRow( $lang['opt_sys_mm'], $lang['opt_sys_mmd'], makeDropDown( array ("php" => "PHP Mail()", "smtp" => "SMTP" ), "save_con[mail_metod]", "{$config['mail_metod']}" ) );
	showRow( $lang['opt_sys_smtph'], $lang['opt_sys_smtphd'], "<input  type=\"text\" name='save_con[smtp_host]' value=\"{$config['smtp_host']}\" style=\"width:100%;max-width:250px\">" );
	...
	showRow( $lang['opt_sys_smtup'], $lang['opt_sys_smtpud'], "<input  type=\"text\" name='save_con[smtp_user]' value=\"{$config['smtp_user']}\" style=\"width:100%;max-width:250px\">" );
	showRow( $lang['opt_sys_smtupp'], $lang['opt_sys_smtpupd'], "<input  type=\"text\" name='save_con[smtp_pass]' value=\"{$config['smtp_pass']}\" style=\"width:100%;max-width:250px\">" );
	...
	showRow( $lang['opt_sys_smtpm'], $lang['opt_sys_smtpmd'], "<input  type=\"text\" name='save_con[smtp_mail]' value=\"{$config['smtp_mail']}\" style=\"width:100%;max-width:250px\">" );
...
elseif( $action == "dosavesyscon" ) {
	...
	while ( $file = readdir( $fdir ) ) {
		if( $file != '.' AND $file != '..' AND $file != '.htaccess' AND $file != 'cron.php' AND strpos($file, "minify_") !== 0 ) {
