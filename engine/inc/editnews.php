...
HTML;
		...
		$entries .= "<td class=\"news-list-tab\" style=\"text-align: center\">";
...
<form class="form-horizontal" action="?mod=editnews&amp;action=list" method="GET" name="optionsbar" id="optionsbar" autocomplete="off">
...
<form action="" method="post" name="editnews" autocomplete="off">
...
<div class="form-group">
	...
	<div class="col-md-10">
		...
		<br /><br />
		<button onclick="clearsubscribe('{$id}'); return false;" class="btn btn-gold"><i class="icon-user"></i> {$lang['btn_clearsubscribe']}</button>
	</div>
</div>
HTML;
...
function clearsubscribe(id) {
    ...
    DLEconfirm( '{$lang['confirm_action']}', '{$lang['p_confirm']}', function () {
		...
		$.get("engine/ajax/adminfunction.php", { id: id, action: 'clearsubscribenews', user_hash: '{$dle_login_hash}' }, function(data){
...
HTML;
    ...
    function sendNotice( id ){
		...
		b['{$lang['p_send']}'] = function() {
			if ( $('#dle-promt-text').val().length < 1) {
				...
				$(this).dialog('close');
				$('#dlepopup').remove();
				$.post('engine/ajax/message.php', { id: id,  text: response, user_hash: '{$dle_login_hash}', allowdelete: \"no\" },
					function(data){
	...
	function find_relates ()
	{
		...
		$.post('engine/ajax/find_relates.php', { title: title, id: '{$row['id']}', user_hash: '{$dle_login_hash}' }, function(data){
	...
	function xffiledelete( xfname, xfvalue )
	{
		...
		DLEconfirm( '{$lang['file_delete']}', '{$lang['p_info']}', function () {
			...
			$.post('engine/ajax/upload.php', { subaction: 'deluploads', user_hash: '{$dle_login_hash}', news_id: '{$row['id']}', author: '{$author}', 'files[]' : xfvalue }, function(data){
...
<div class="box">
            ...
            <div class="box-content">
			<form method="post" class="form-horizontal" name="addnews" id="addnews" onsubmit="if(checkxf()=='fail') return false;" action="" autocomplete="off">
                     ...
                     <div class="tab-pane active" id="tabhome">
						<div class="row box-section">
							...
							<div class="form-group">
							  ...
							  <div class="col-md-10">
								<input type="text" name="newdate" data-rel="calendar" size="20" value="{$row['date']}">&nbsp;<input class="checkbox-inline" type="checkbox" name="allow_now" id="allow_now" value="yes">&nbsp;<label for="allow_now">{$lang['edit_jdate']}</label>
							  </div>
							 </div>
	...
	if( $item_db[0] ) {
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				$db->query( "DELETE FROM " . PREFIX . "_subscribe WHERE news_id='{$item_db[0]}'");
				$db->query( "UPDATE " . USERPREFIX . "_users SET news_num=news_num-1 WHERE name='{$item_db[1]}'" );
				deletecommentsbynewsid( $item_db[0] );
				$okdeleted = TRUE;
