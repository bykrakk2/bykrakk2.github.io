...
class ParseFilter {
			...
			if ($this->is_comments) {
				...
				if( intval( $config['auto_wrap'] ) ) {
					...
					for($i = 0; $i < $n; $i ++) {
						...
						if( preg_match( "#([^\s\n\r]{" . intval( $config['auto_wrap'] ) . "})#{$utf_pref}i", $source[$i] ) ) {
			
							$source[$i] = preg_replace( "#([^\s\n\r]{" . intval( $config['auto_wrap']-1 ) . "})#{$utf_pref}i", "\\1<br />", $source[$i] );
	...
	function build_url( $matches=array() ) {
		...
		if( stripos( $url['html'], "engine/go.php" ) !== false ) {
			return $matches[0];
		}

		if( !preg_match( "#^(http|https|ftp|nntp|news|tel)://#", $url['html'] ) AND $url['html'][0] != "/" AND $url['html'][0] != "#") {
	...
	function build_thumb( $matches=array() ) {
		...
		if( $align == '' ) return "<!--TBegin:{$info}--><a href=\"$gurl\" class=\"highslide\" target=\"_blank\"><img src=\"$url\" style=\"max-width:100%;\" {$alt}></a><!--TEnd-->";
		else return "<!--TBegin:{$info}--><a href=\"$gurl\" class=\"highslide\" target=\"_blank\"><img src=\"$url\" style=\"float:{$align};max-width:100%;\" {$alt}></a><!--TEnd-->";
	...
	function build_medium( $matches=array() ) {
		...
		if( $align == '' ) return "<!--MBegin:{$info}--><a href=\"$gurl\" class=\"highslide\"><img src=\"$url\" style=\"max-width:100%;\" {$alt}></a><!--MEnd-->";
		else return "<!--MBegin:{$info}--><a href=\"$gurl\" class=\"highslide\"><img src=\"$url\" style=\"float:{$align};max-width:100%;\" {$alt}></a><!--MEnd-->";
	...
	function clear_url($url) {
		...
		$url = preg_replace( "/d&#1072;ta:data(.*?):/i", "data:", $url );
		$url = htmlspecialchars( $url, ENT_QUOTES, $config['charset'] );
		
		$url_array = parse_url($url);
		
		if ( $url_array['scheme'] AND !in_array( $url_array['scheme'], array("http","https","mailto","ftp","nntp","news","tel")) ) {

			return '';
		}
		
		if( stripos( $url, "engine/go.php" ) !== false ) {
			return '';
		}
	...
	function remove_bad_url( $matches=array() ) {
		...
		if( preg_match( "#href=['\"](.+?)['\"]#i", $params, $match ) ) {
			...
			if( stripos( $match[1], "engine/go.php" ) !== false ) {
				return '';
			}
...
class OEmbed {
	...
	function __construct(){
		...
		$this->providers['%^(http:|https:)?//(www.)?(facebook.com/)%i'] = "https://www.facebook.com/plugins/video/oembed.json/";
	...
	function toHtml($data, $args){
		global $config;
 		...
 		if(is_object($data) || !empty($data->type)){
			...
			$html = convert_unicode($html, $config['charset']);
