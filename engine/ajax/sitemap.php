...
if ( strtolower($config['charset']) != "utf-8" ) {
	...
	if( function_exists( 'mb_convert_encoding' ) ) {
		...
		$map_link = mb_convert_encoding( $map_link, "UTF-8", $config['charset'] );

	} elseif( function_exists( 'iconv' ) ) {
	
		$map_link = iconv($config['charset'], "UTF-8//IGNORE", $map_link);
...
if (strpos ( send_url("https://google.com/webmasters/sitemaps/ping?sitemap=", $map_link), "successfully added" ) !== false) {
	...
	$buffer .= "<br />".$lang['sitemap_send']." Google: ".$lang['nl_error']." URL: <a href=\"https://google.com/webmasters/sitemaps/ping?sitemap=".urlencode($map_link)."\" target=\"_blank\">http://google.com/webmasters/sitemaps/ping?sitemap={$map_link}</a>";
...
if (strpos ( send_url("https://ping.blogs.yandex.ru/ping?sitemap=", $map_link), "OK" ) !== false) {
	...
	$buffer .= "<br />".$lang['sitemap_send']." Yandex: ".$lang['nl_error']." URL: <a href=\"https://ping.blogs.yandex.ru/ping?sitemap=".urlencode($map_link)."\" target=\"_blank\">http://ping.blogs.yandex.ru/ping?sitemap={$map_link}</a>";
