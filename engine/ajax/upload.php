...
if( $_POST['subaction'] == "deluploads" ) {
	...
	if( $user_group[$member_id['user_group']]['allow_up_image'] AND count( $_POST['comments_files'] ) ) {
...
jQuery(function($){
	...
	var uploader = new qq.FileUploader({
	    ...
	    params: {"PHPSESSID" : "{$sess_id}", "subaction" : "upload", "news_id" : "{$news_id}", "area" : "{$area}", "author" : "{$author}", "user_hash" : "{$dle_login_hash}"},
...
function insertcontent( content ) {
	...
	if ( wysiwyg == '1' ) {
		...
		if(allways_bbimages == '1') {
			...
			parent.active_editor.html.insert( content +'<br>' + parent.$.FE.MARKERS );
		}
		parent.active_editor.undo.saveStep();
...
function upload_from_url( url ) {
	...
	$.post( "{$root}engine/ajax/upload.php", { news_id: "{$news_id}", imageurl: copyurl, ftpurl: ftpurl, t_size: t_size, t_seite: t_seite, make_thumb: make_thumb, m_size: m_size, m_seite: m_seite, make_medium: make_medium, make_watermark: make_watermark, area: "{$area}", author: "{$author}", subaction: "upload", user_hash : "{$dle_login_hash}" }, function(data){
